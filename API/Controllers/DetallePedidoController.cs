...
public class DetallePedidoController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<DetallePedidoDto>>> GetPag([FromQuery] Params resultParams)
    {
        var result = await _unitOfWork.DetallePedidos.GetAllAsync(resultParams.PageIndex, resultParams.PageSize, resultParams.Search);
        var lstResultDto = _mapper.Map<List<DetallePedidoDto>>(result.registros);
        return new Pager<DetallePedidoDto>(lstResultDto, result.totalRegistros, resultParams.PageIndex, resultParams.PageSize, resultParams.Search);
    }
}
