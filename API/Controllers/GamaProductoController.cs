...
public class GamaProductoController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<GamaProductoDto>>> GetPag([FromQuery] Params resultParams)
    {
        var result = await _unitOfWork.GamaProductos.GetAllAsync(resultParams.PageIndex, resultParams.PageSize, resultParams.Search);
        var lstResultDto = _mapper.Map<List<GamaProductoDto>>(result.registros);
        return new Pager<GamaProductoDto>(lstResultDto, result.totalRegistros, resultParams.PageIndex, resultParams.PageSize, resultParams.Search);
    }
}
